...
exports.findAllAttendances =(req, res) => {
  ...
  Attendance.findAllAttendances(attendanceId, (err, data) => {
...
exports.findOneAttendance = (req, res) => {
    Attendance.findAttendanceById(req.params.attendanceId, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found attendance with id ${req.params.attendanceId}.`
        });
      } else {
        res.status(500).send({
          message: "Error retrieving attendance with id " + req.params.attendanceId
        });
      }
    } else res.send(data);
  });
...
exports.deleteAttendance = (req, res) => {
    Attendance.deleteAttendance(req.params.attendanceId, 
    (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
        ...
        res.status(500).send({
          message: "Could not delete Attendance with id " + req.params.attendanceId
        });
      }
