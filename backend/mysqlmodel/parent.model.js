...
Parent.findChildrenExamByparentId = (parentId, result) => {
  sql.query(`SELECT m.subjectCode, u.subjectName, m.student_score, o.totalScore, round((m.student_score/o.totalScore)*100,2) as grade, s.studentId ,CASE WHEN (round((m.student_score/o.totalScore)*100,2))>=80 THEN 'A=distinction' WHEN (round((m.student_score/o.totalScore)*100,2))>=70 THEN 'B=very good' WHEN (round((m.student_score/o.totalScore)*100,2))>=60 THEN 'C=good' WHEN (round((m.student_score/o.totalScore)*100,2))>=50 THEN 'D=average' ELSE 'F=fail' END AS remarks from students s inner join classes c on s.classId=c.classId join student_marks m on m.studentId = s.studentId JOIN parents p on s.parentId=p.parentId join subjects u on m.subjectCode=u.subjectCode join totals o on o.subjectCode=u.subjectCode join terms t on m.termId=t.termId where p.parentId like '${parentId}' order by s.studentId;`, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
      return;
